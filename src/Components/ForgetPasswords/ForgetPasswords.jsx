            ...
            <div className="flex justify-center items-start -mt-20">
                ...
                <div className={style.formContainer}>
                    <form onSubmit={handleSubmit} className="w-full flex flex-col p-10 rounded-2xl gap-4 bg-body dark:bg-dark">
                        <h1 id="heading" className="text-xl pt-0 pb-4 font-bold dark:text-white text-[#240253] text-center">Forget Password</h1>
                        ...
                        <div className="mt-3 bg-transparent border-0 hover:bg-transparent flex justify-center items-center flex-row">
                            <button type="submit" className="button1 text-white rounded-md shadow-sm w-full  py-3 px-11 font-semibold disabled:bg-gray-600" disabled={isLoading}>Reset password {isLoading && <i className="fa-solid fa-spinner fa-spin ms-1"></i>}</button>

                            {/* <button className="button2 text-white rounded-md shadow-sm  py-3 px-11 font-semibold"><Link to={"/register"} className=" text-white" >Sign Up</Link></button> */}
                        </div>
                        {errorMsg && <p className='text-red-500 text-center'>{errorMsg}</p>}
                        {succussMsg && <p className='text-green-500 text-center'>{succussMsg} </p>}
                        <Link to={"/login"} className="text-center text-[#240253] dark:text-white link link-hover transition-all mt-3">Remember your password?</Link>
                    </form>
                </div>
