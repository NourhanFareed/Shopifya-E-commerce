...
export default function ResetPassword() {
    ...
    const validationSchema = Yup.object({
        ...
        newPassword: Yup.string().required("Password is required").matches(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/, "Password length must be at least 8 characters long and contain at least one letter, one number and one special character"),
    ...
    async function onSubmit() {
        ...
        await axios.put("https://ecommerce.routemisr.com/api/v1/auth/resetPassword", values).then((response) => {
            setIsLoading(false);
                    ...
                    <form onSubmit={handleSubmit} className="w-full flex flex-col p-10 rounded-2xl gap-4 bg-body dark:bg-dark">
                        ...
                        <div className="flex justify-center items-center flex-col w-full">
                            <div className="field my-1 w-full bg-body shadow-4xl dark:shadow-3xl dark:bg-dark pt-2">
                                ...
                                <input onBlur={handleBlur} onChange={handleChange} value={values.newPassword} type="password" id="newPassword" name="newPassword" placeholder='New Password' className="w-full px-3 py-1 border-none bg-transparent text-black dark:text-white focus:outline-none focus:border-none" />
                            </div>
                            {touched.newPassword && errors.newPassword && <p className='text-red-500 pt-2 ps-2'>{errors.newPassword}</p>}
                        </div>
                        <div className="mt-3 bg-transparent border-0 hover:bg-transparent flex justify-center items-center flex-row">
                            <button type="submit" className="button1 text-white rounded-md shadow-sm block w-full text-[15px]  py-3 px-11 font-semibold disabled:bg-gray-600" disabled={isLoading}>Reset {isLoading && <i className="fa-solid fa-spinner fa-spin ms-1"></i>}</button>
